...
constexpr int   MMPROJ_MAX_IMAGE_TOKENS = 512;
constexpr int   MMPROJ_BATCH_MAX_TOKENS = 512;
...
// Multimodal (vision) support. Null unless a mmproj projector was
// successfully loaded for the current model via loadMultimodalProjector().
...
/**
 * Load a mmproj (vision projector) file for the currently loaded model, enabling
 * it to process images. Must be called AFTER load()/prepare(). Only "VL"-type
 * models ship a matching mmproj gguf (usually a separate download).
 *
 * Returns 0 on success, 1 if no model is loaded yet, 2 if the projector failed
 * to load (wrong/incompatible file).
 */
extern "C"
JNIEXPORT jint JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_loadMultimodalProjectorNative(
        JNIEnv *env, jobject /*unused*/, jstring jmmproj_path) {
    if (!g_model) {
        LOGe("%s: model must be loaded before the mmproj projector", __func__);
        return 1;
    }
    ...
    mtmd_params.use_gpu = false; // CPU-only backends on-device for now
    ...
    // Cap image resolution tokens and mtmd's internal batch size to reduce peak
    // memory during vision encoding - important on memory-constrained phones,
    // where the vision encoder pass runs on top of the already-loaded model.
    // Qwen-VL-style models otherwise scale token count with image resolution,
    // which can spike memory a lot for large photos.
    mtmd_params.image_max_tokens = MMPROJ_MAX_IMAGE_TOKENS;
    mtmd_params.batch_max_tokens = MMPROJ_BATCH_MAX_TOKENS;

    auto *mtmd_ctx = mtmd_init_from_file(mmproj_path, g_model, mtmd_params);
    env->ReleaseStringUTFChars(jmmproj_path, mmproj_path);

    if (!mtmd_ctx) {
        LOGe("%s: Failed to init mtmd context from mmproj file", __func__);
        return 2;
    }
    ...
    g_mtmd_ctx = mtmd_ctx;
    LOGi("%s: mmproj loaded, vision support enabled", __func__);
    return 0;
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_supportsVisionNative(JNIEnv * /*env*/, jobject /*unused*/) {
...
/**
 * Same as processUserPrompt(), but also feeds an image through the loaded
 * mmproj vision projector so a VL model can actually "see" it.
 *
 * Requires loadMultimodalProjector() to have succeeded beforehand.
 * Returns: 0 success, 1 vision not available/no projector loaded,
 *          2 tokenize/marker mismatch, 3 decode failure.
 */
extern "C"
JNIEXPORT jint JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_processUserPromptWithImage(
    ...
    if (!g_mtmd_ctx || !mtmd_support_vision(g_mtmd_ctx)) {
        LOGe("%s: no vision-capable mmproj loaded", __func__);
        return 1;
    }

    // Read the raw image bytes (still-encoded jpg/png/etc, decoded internally by mtmd via stb_image)
    const jsize image_len = env->GetArrayLength(jimage_bytes);
    std::vector<unsigned char> image_bytes(image_len);
    env->GetByteArrayRegion(jimage_bytes, 0, image_len, reinterpret_cast<jbyte *>(image_bytes.data()));

    auto bitmap_wrapper = mtmd_helper_bitmap_init_from_buf(g_mtmd_ctx, image_bytes.data(), image_bytes.size(),
                                                             /*placeholder*/ false);
    ...
    // Obtain and format the user prompt with the media marker embedded, so the
    // chat template wraps around both the text AND the image placeholder.
    const auto *const user_prompt = env->GetStringUTFChars(juser_prompt, nullptr);
    std::string raw_prompt(user_prompt);
    env->ReleaseStringUTFChars(juser_prompt, user_prompt);

    const std::string marker = mtmd_get_marker(g_mtmd_ctx);
    std::string content_with_marker = marker + "\n" + raw_prompt;
    LOGd("%s: User prompt (with image) received: \n%s\n", __func__, content_with_marker.c_str());

    const bool has_chat_template = common_chat_templates_was_explicit(g_chat_templates.get());
    std::string formatted_user_prompt = has_chat_template
            ? chat_add_and_format(ROLE_USER, content_with_marker)
            : content_with_marker;

    mtmd_input_text input_text{
            /*text*/ formatted_user_prompt.c_str(),
            /*text_len*/ formatted_user_prompt.size(),
            /*add_special*/ has_chat_template,
            /*parse_special*/ has_chat_template,
    };
    ...
    const int32_t eval_result = mtmd_helper_eval_chunks(
            g_mtmd_ctx, g_context, chunks, current_position,
            /*seq_id*/ 0, BATCH_SIZE, /*logits_last*/ true, &new_n_past);
    mtmd_input_chunks_free(chunks);
    ...
    const llama_pos consumed = new_n_past - current_position;
    current_position = new_n_past;
    stop_generation_position = current_position + consumed + n_predict;
    return 0;
}
